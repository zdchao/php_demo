	$(document).ready(function() {
		...
		var RecordClass = function() {
			...
			this.getRecordList = function(page,showloading) {
			...
			this.renderOne = function(key) {
				...
				var operat = div.find("#record-operat");

				if (row['status']=='enable') {

					operat.find("#stop").remove();

					operat.find("#restor").bind({
					...
					operat.find("#restor").remove();

					operat.find("#stop").bind({
				...
				if (parseInt(row['backup'])==1) {
					...
					operat.find("#nobackup").remove();

					operat.find("#backup").bind({
					...
					operat.find("#backup").remove();

					operat.find("#nobackup").bind({
				...
				var monitordiv = div.find('#record-monitor');

				if (row['monitor_enable']) {

					if(row['monitor_flag'] == 0){

						monitordiv.find("#normal").remove();

					}

					if (row['monitor']) {

						monitordiv.find("#add").remove();

						

						monitordiv.find("#del").bind({
						...
						monitordiv.find("#normal").bind({

							'mouseover':function(){$(this).tooltip('show');}

						});

						monitordiv.find('#error,#switch').bind({
						...
						if (!row['monitor_switch']) {
							...
							monitordiv.find('#switch').remove();

						}else {

							monitordiv.find("#switch").bind('click',function(){
						...
						if (!row['monitor_error']) {
							...
							monitordiv.find('#error').remove();

						}else {

							monitordiv.find("#error").bind('click',function(){
						...
						if (!row['monitor_switch'] && !row['monitor_error']) {
							...
							monitordiv.find("#normal").bind('click',function(){
						...
						monitordiv.find("#del,#edit,#error,#switch,#normal").remove();

						monitordiv.find("#add").bind({
				...
				if(that.iscdn == 1){
					...
					var nocdntypes = ["DNAME","MX","NS","TXT","SRV","URL",'CNAME'];

					if(that.cdn_id == 0 || inArray(row['type'],nocdntypes)){

						operat.find("#domain_status0,#domain_status1,#edit_cdn").remove();

					}

					operat.find("#domain_status0").bind({
					...
					if(row['cdn'] == 0){
						...
						operat.find("#domain_status1,#edit_cdn").remove();

					}else{

						operat.find("#domain_status0").remove();

						operat.find("#domain_status1").bind({
			...
			*/
			...
			this.piaoAddMonitor = function(key) {
				...
				var url =  'http://'+sub_domain+that.domain+'/';
				...
				mdiv = $("#monitor-add-row");

				mdiv.find('button').eq(0).bind('click',function(){

					that.removePiaoAddMonitor();

				});

				mdiv.find('button').eq(1).bind('click',function(){

					that.addMonitor(key);

				});

				if(mdiv.find(':radio[name="action"]:checked').val()!=2){

					mdiv.find("#switch_type,#switch_host").hide();

				}

				mdiv.find("[name=monitor_t]").bind('click',function(){

					that.showActiveValuePlaceholder($(this).val(),mdiv);

				});

				mdiv.find("[name=monitor_type]").bind('click',function(){

					that.showChangeMonitorType($(this).val(),mdiv,url);

				});

				mdiv.find("input[name='action']").change(function(){

					if($(this).val()!=2){

						mdiv.find("#switch_type,#switch_host").hide("fast");

					}else{

						mdiv.find("#switch_type,#switch_host").show("fast");

					}

				});

				this.showActiveValuePlaceholder(mdiv.find("[name=monitor_t]").val(),mdiv);

				this.showChangeMonitorType(mdiv.find("[name=monitor_type]").val(),mdiv,url);

			};

			this.showChangeMonitorType = function(type,mdiv,url){
				...
				case "tcp":
					...
					mdiv.find("#http_content").hide();

					mdiv.find("[name=url]").attr('value',"");

					mdiv.find("[name=url]").attr('placeholder',"tcp://:80");
				...
				case "http":
					...
					mdiv.find("[name=url]").attr('value',url);

					break;

				case "http_body":

					mdiv.find("#http_content").show();

					mdiv.find("[name=url]").attr('value',url);

					mdiv.find("[name=content]").attr('placeholder',"监控网址中包含的内容");
			...
			this.showActiveValuePlaceholder = function(type,mdiv){

				var v = mdiv.find("[name=value]");

				switch (type) {

					case "A":

						v.attr('placeholder',"192.168.1.11");

						break;

					case "AAAA":

						v.attr('placeholder',"fe80::20c:29ff:fe00:73e2");

						break;

					case "CNAME":

						v.attr('placeholder',"www."+this.domain + ".");
			...
			this.addMonitor = function(key) {
				...
				if(mdiv.find('[name=monitor_type]:checked').val() == "http_body"){

					if(!content){

						that.showError('监控内容不能为空',true);
				...
				$.ajax({
					...
					success:function(a) {
						...
						if(weixin != undefined){
							...
							notice["notice_weixin"] = true;

							that.updateRecordMonitorData(key,notice);

						}

						if(sms != undefined){

							notice["notice_sms"] = true;
			...
			this.piaoEditMonitor = function(key) {
				...
				var mdiv = 	$("#monitor-edit-row");

				if(data['content']){

					mdiv.find("#http_content").show();

					mdiv.find('[name=monitor_type]').eq(1).attr('checked',"checked");

				}else{

					mdiv.find("#http_content").hide();

					if(data['url'].indexOf('tcp')<0){

						mdiv.find('[name=monitor_type]').eq(0).attr('checked',"");

					}else{

						mdiv.find('[name=monitor_type]').eq(2).attr('checked',"");
				...
				this.showChangeMonitorType(mdiv.find("[name=monitor_type]").val(),mdiv,data['url']);
				...
				mdiv.find('#esc').bind('click',function(){

					that.removePiaoEditMonitor();

				});

				mdiv.find('#enter').bind('click',function(){
			...
			this.editMonitor = function(key) {
				...
				if(div.find('[name=monitor_type]:checked').val() == "http_body"){
			...
			this.piaoAddRecord = function() {
				...
				adddiv.find('#enter').bind('click', function() {
				...
				adddiv.find('#esc').bind('click', function() {
			...
			this.piaoAddPrompt = function(obj) {
				...
				var divtemp = $("#prompt-"+id+"-div").html();
			...
			this.piaoAddMultiRecord = function() {
				...
				this.multidiv.find("select[name=record_type]").change(function(){

					var type = that.multidiv.find("option:selected").val();

					that.changeShow(type);

				});

			};

			this.changeShow = function(type){
				...
				case "AAAA":
					...
					content = "ff06:0:0:0:0:0:0:c3";

					break;

				case "CNAME":

					content = "www."+this.domain;
				...
				case "MX":
					...
					content = "10 mail."+this.domain;

					break;

				case "NS":

					content = "ns1."+this.domain;
				...
				case "SRV":
					...
					content = "5 0 5269 srv."+this.domain;

					break;

				case "URL":

					content = "301 url."+this.domain;
				...
				this.multidiv.find("[name=values]").attr('placeholder',"www "+content);
			...
			this.addMultiRecord = function() {
				...
				this.replace_add = replace ? true : false;
				...
				$.ajax({
					...
					success:function(a) {
						...
						if (parseInt(a.status.code) != 1) {
							...
							that.multiresult.success.push(a.record.name +':'+a.status.message);
			...
			this.showImportList = function() {
				...
				operatrowhtml += "<button class='btn btn-primary' id='enter' data-loading-text='正在执行..'>确定</button>�<button class='btn btn-primary'>取消</button>";
