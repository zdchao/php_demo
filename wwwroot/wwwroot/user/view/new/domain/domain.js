		...
		function domain() {
			...
			this.renderGroup = function() {
				...
				if (that.grouplist.length >0) {
					...
					$("#dropdown_group").find('a').not("#createa").unbind();
			...
			this.changeGroup = function(gkey) {
				...
				this.selectarr = select.sort(function(a,b){return b-a});
				this.deferrChangeGroup(0,gkey);
			}
			this.deferrChangeGroup = function(selectkey,gkey)	{
				if (selectkey >= this.selectarr.length) {
					this.showMultiResult();
					return;
				}
				this.deferrChangeOneGroup(selectkey,gkey);
			}
			this.deferrChangeOneGroup = function(selectkey,gkey) {
				var rowkey = this.selectarr[selectkey];
				var row = this.getRow(rowkey);
				var grow = this.getGroupRow(gkey);
				if (row['group_id'] == grow['group_id']) {
					this.deferrChangeGroup(selectkey+1,gkey);
					return;
				}
				var that = this;
				$.ajax({
					...
					success:function(a) {
						...
						that.deferrChangeGroup(selectkey+1,gkey);
					},
					error:function(e) {
						that.multiresult.error.push(row['name'] + ' 服务器数据出错');
						that.deferrChangeGroup(selectkey+1,gkey);
					}
				});
			}
